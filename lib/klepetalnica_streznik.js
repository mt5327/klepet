  ...
  socket.on('dregljaj', function (dregljaj) {
    if (dregljaj.vzdevek) {
      ...
      if (socketIdNaslovnika && socketIdNaslovnika != socket.id) {
       ...
       io.sockets.sockets[socketIdNaslovnika].emit('obvestilo', { 
         besedilo: vzdevkiGledeNaSocket[socket.id] + " vas je dregnil(a)" 
       });
      } else if (socketIdNaslovnika == socket.id)  {
       io.sockets.sockets[socket.id].emit('obvestilo', {
         besedilo: "Samemu sebi ne moreš poslati dregljaja."
       });
      } else {
       io.sockets.sockets[socket.id].emit('obvestilo', {
